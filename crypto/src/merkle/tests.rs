...
fn verify() {
    ...
    assert_eq!(true, super::MerkleTree::verify(tree.root(), 1, &proof, hash::blake3));

    let proof = tree.prove(2);
    assert_eq!(true, super::MerkleTree::verify(tree.root(), 2, &proof, hash::blake3));
    ...
    assert_eq!(true, super::MerkleTree::verify(tree.root(), 6, &proof, hash::blake3));
}
...
fn verify_batch() {
    ...
    let proof = tree.prove_batch(&[1]);
    assert_eq!(true, super::MerkleTree::verify_batch(tree.root(), &[1], &proof, hash::blake3));
    assert_eq!(false, super::MerkleTree::verify_batch(tree.root(), &[2], &proof, hash::blake3));

    let proof = tree.prove_batch(&[1, 2]);
    assert_eq!(true, super::MerkleTree::verify_batch(tree.root(), &[1, 2], &proof, hash::blake3));
    assert_eq!(false, super::MerkleTree::verify_batch(tree.root(), &[1], &proof, hash::blake3));
    assert_eq!(false, super::MerkleTree::verify_batch(tree.root(), &[1, 3], &proof, hash::blake3));
    assert_eq!(false, super::MerkleTree::verify_batch(tree.root(), &[1, 2, 3], &proof, hash::blake3));

    let proof = tree.prove_batch(&[1, 6]);
    assert_eq!(true, super::MerkleTree::verify_batch(tree.root(), &[1, 6], &proof, hash::blake3));

    let proof = tree.prove_batch(&[1, 3, 6]);
    assert_eq!(true, super::MerkleTree::verify_batch(tree.root(), &[1, 3, 6], &proof, hash::blake3));

    let proof = tree.prove_batch(&[0, 1, 2, 3, 4, 5, 6, 7]);
    assert_eq!(true, super::MerkleTree::verify_batch(tree.root(), &[0, 1, 2, 3, 4, 5, 6, 7], &proof, hash::blake3));
}
