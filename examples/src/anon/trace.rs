...
pub fn generate_trace(
    ...
    let mut trace = vec![
        ...
        vec![FieldElement::ZERO; trace_length], // merkle branch hash state
        vec![FieldElement::ZERO; trace_length], // merkle branch hash state
        vec![FieldElement::ZERO; trace_length], // merkle branch hash state
        vec![FieldElement::ZERO; trace_length], // merkle branch hash state
        vec![FieldElement::ZERO; trace_length], // sub-token hash state
        vec![FieldElement::ZERO; trace_length], // sub-token hash state
        vec![FieldElement::ZERO; trace_length], // sub-token hash state
        vec![FieldElement::ZERO; trace_length], // sub-token hash state
    ];
    ...
    for step in 0..(trace_length - 1) {
        ...
        if cycle_pos < NUM_HASH_ROUNDS {
            ...
            state[0] = FieldElement::from(((token_index >> cycle_num) & 1) as u128);
            ...
            state[4] = FieldElement::ZERO;
            state[5] = FieldElement::ZERO;
            state[6] = FieldElement::ZERO;
            state[7] = FieldElement::ZERO;
            state[8] = FieldElement::ZERO;
