...
impl<H: ElementHasher> RamConstraintProver<H> {
    ...
    pub fn build_trace(&self, valid_ram: &Vec<[u64; 4]>) -> TraceTable<BaseElement> {
        ...
        let log_ram_steps_u64: u64 = log2(self.num_ram_steps).try_into().unwrap();
        let log_ram_locs_u64: u64 = log2(self.num_locs).try_into().unwrap();
        let trace_width = 4 + log_ram_locs_usize + log_ram_steps_usize + 4;
        ...
        trace.fill(
            |state| {
                ...
                state[1] = BaseElement::from(valid_ram[0][1]);
