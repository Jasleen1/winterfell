...
use winterfell::{
    math::{log2, StarkField, ToElements},
...
impl Air for FFTRapsAir {
    ...
    fn evaluate_transition<E: FieldElement + From<Self::BaseField>>(
        ...
        for i in 0..num_steps {
            result[2 * num_steps + 3 + i] = are_equal(
                current[last_base_col + 1 + i] * current[last_base_col + 1 + i],
                current[last_base_col + 1 + i],
            );
        }
    }
    ...
    fn get_assertions(&self) -> Vec<Assertion<Self::BaseField>> {
        ...
        let mut assertions = vec![Assertion::single(num_cols - 2, 0, BaseElement::ZERO)];
    ...
    fn get_aux_assertions<E: FieldElement + From<Self::BaseField>>(
        ...
        for step in 2..num_steps + 1 {
            ...
            output_vec.push(Assertion::single(
                3 * num_steps + 3 * step - 4,
                last_step,
                E::ONE,
            ));
        }
    ...
    fn get_periodic_column_values(&self) -> Vec<Vec<Self::BaseField>> {
        ...
        for j in 2..num_steps + 1 {
            ...
            let jump_field_elt = BaseElement::new(2).exp(
