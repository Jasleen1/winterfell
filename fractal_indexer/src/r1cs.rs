...
pub fn valid_matrix<E: FieldElement>(name: &str, matrix: Vec<Vec<E>>) -> Result<Matrix<E>, R1CSError> {
    let rows = matrix.len();
    if rows == 0 {
        let dims = (0, 0);
        return Ok(Matrix{name: String::from(name), mat: matrix, dims: dims});
    }
    else {
        ...
        let dims = (rows, cols);
        return Ok(Matrix{name: String::from(name), mat: matrix, dims});
    }
}
...
impl<E: FieldElement> R1CS<E> {
    pub fn new(matrix_a: Matrix<E>, matrix_b: Matrix<E>, matrix_c: Matrix<E>) -> Result<Self, R1CSError> {
        let valid = valid_r1cs(&matrix_a, &matrix_b, &matrix_c);
        match valid {
            Ok(_) => {Ok(R1CS {A: matrix_a, B: matrix_b, C: matrix_c})},
            Err(e) => Err(e),
        }
    } 
...
pub fn valid_r1cs<E: FieldElement>(a: &Matrix<E>, b: &Matrix<E>, c: &Matrix<E>) -> Result<bool, crate::errors::R1CSError> {
    let a_dims = a.dims;
    let b_dims = b.dims;
    let c_dims = c.dims;
    if b_dims != a_dims {
        return Err(R1CSError::MatrixSizeMismatch(a.name.clone(), b.name.clone()));
    }
    else if c_dims != a_dims {
        return Err(R1CSError::MatrixSizeMismatch(a.name.clone(), c.name.clone()));
    }
    else {
        return Ok(true);
    }
