use crypto::{ElementHasher, RandomCoin, MerkleTree};
use fractal_indexer::snark_keys::VerifierKey;
use math::StarkField;

use fri::{DefaultVerifierChannel, FriVerifier, VerifierError};

use fractal_proofs::{FieldElement, RowcheckProof, FOLDING_FACTOR, get_complementary_poly, polynom, TryInto};
...
>(
    ...
    MerkleTree::verify_batch(&proof.s_eval_root, &proof.queried_positions.clone(), &s_original_proof).map_err(|err| VerifierError::MerkleTreeErr(err))?;
...
>(eval_domain_size: usize, original_degree: usize, max_degree: usize, 
    original_evals: Vec<E>, final_evals: Vec<E>, positions: Vec<usize>) -> Result<(), VerifierError> {
    ...
    for (pos, _) in eval_domain_elts.iter().enumerate() {
        if original_evals[pos].mul(eval_domain_evals[pos]) != final_evals[pos] {
            ...
            return Err(VerifierError::SmallPolyAdjustmentErr);
        }
    }
    Ok(())
}
