...
pub fn add_in_place(a: &mut [BaseElement], b: &[BaseElement]) {
    assert!(
        a.len() == b.len(),
...
pub fn mul_acc(a: &mut [BaseElement], b: &[BaseElement], c: BaseElement) {
    assert!(
        a.len() == b.len(),
...
pub fn infer_degree(evaluations: &[BaseElement]) -> usize {
    assert!(
        evaluations.len().is_power_of_two(),
        "number of evaluations must be a power of 2"
    );
    let mut poly = evaluations.to_vec();
    let root = BaseElement::get_root_of_unity(evaluations.len().trailing_zeros());
